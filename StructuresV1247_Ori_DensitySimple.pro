...
;  diskcase='alpha_visc-'+npStr+'_planets'		;The case
  dataFolder='~/fargo3d_outputs/V1247_Ori-Simple'
...
;  nrad = uint(dims(6))
  nsec=uint(nz)
  nrad=uint(nx)
;print,densgas/(2./2.25*1.e7)
dens_initial=densgas/(2./2.25*1.e7)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;      vxPlanet[i]=planet[4,output]
;;      vyPlanet[i]=planet[5,output] 
;;      mPlanet[i]=planet[7,output]*1.86
;;      close,1
;;   endfor
;; ;print,mPlanet
...
;  tau_flag[0]=0.1
  tau_flag[0]=0.5
  tau_flag[1]=2.0
  tau_flagStr=strarr(TauRanges-1)
;  tau_flagStr[0]='0.1'
  tau_flagStr[0]='0.5'
  tau_flagStr[1]='2.0'

;  tauInnerStr=strtrim(tauInner,2)
;  tauOuterStr=strtrim(tauOuter,2)
...
;  close,1
rad=xi1D/AU
  rmin=rad(0);InnerBorderAU;rad(0)
  rmax=rad(nrad);sizeAU;rad(nrad)
;print,rmin,rmax
;print,InnerBorderAU,sizeAU
...
;     if ((rad[i] ge (Rplanet[0]-(2*HillPlanet[0]))) and (rad[i] le (Rplanet[nPlanets-1]-(2*HillPlanet[nPlanets-1])))) then begin
     if (rad[i] ge InnerGapBorderAU) and (rad[i] le OuterGapBorderAU) then begin
        for j=0,nsec-1 do begin
          tau=9.6*dens[j,i]*2./2.25*1.e7 ;Optical depth Troilita,etc.[the density is in cgs]              
...
;  MeanFillingFactorAtGap(3)=Atau(3)/total(ATau(*))
;  MeanFillingFactorAtGap(4)=Atau(4)/total(ATau(*))
